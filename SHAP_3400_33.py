...
# ✅ LOAD CSV + TAKE EXACTLY 3,400 ROWS (matches model!)
tickets = pd.read_csv('joined_data.csv')
tickets = tickets.iloc[:3400]  # ✅ THE MAGIC NUMBER FROM ERROR!
print(f'✅ USING EXACTLY: {len(tickets)} rows')

# ✅ BUILD ALL 33 COLUMNS — fill missing with 0
X = pd.DataFrame()
for name in feature_names:
    if name in tickets.columns:
        X[name] = pd.to_numeric(tickets[name], errors='coerce').fillna(0).astype(int)
    else:
        X[name] = 0  # ✅ Missing columns = 0
        print(f'⚠️ Auto-filled: {name}')

print(f'✅ FINAL SHAPE: {X.shape} — SHOULD BE (3400, 33)')
